...
Format: `add type/s n/NAME type/TYPE p/PHONE_NUMBER e/EMAIL a/ADDRESS [t/TAG]…​`
...
Examples:
* `add type/s n/John Doe type/p p/98765432 e/johnd@example.com a/John street, block 123, #01-01`
* `add type/s n/Betsy Crowe type/s e/betsycrowe@example.com a/Newgate Prison p/1234567 t/criminal`
...
* Unlinks the student from the parent at the specified `INDEX`s.
* The index refers to the index number shown in the displayed person list.
* The index **must be a positive integer** 1, 2, 3, …​
...
* COST must be a numeric value, eg. 72.5
* To edit the COST of a specific student, simply type EDIT INDEX . . . pay/COST*


Examples
‘Add type/s n/malcolm . . . pay/100’ - Adds a student named malcolm whose cost per lesson is $100
...
*list — Displays everyone in the address book.
*list paid — Displays only contacts marked as Paid.
*list unpaid — Displays only contacts marked as Unpaid.


Error handling:


*If you pass anything other than paid or unpaid, the app shows a clear error with the correct usage


### Tracking payment status of each Student : `paid`


Toggles payment status of specified contact
...
*Toggles payment status of specified contact between paid/unpaid
